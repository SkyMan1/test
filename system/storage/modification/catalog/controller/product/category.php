...
MegaFilter.prototype = {
	...
	*/
	...
	/**
	 * Trwa oczekiwanie na odpowiedz serwera
	 * 
	 * @var bool
	 */
	_busy			: false,
	...
	/**
	 * W trakcie ladowania danych z serwra wprowadzono zmiany
	 * 
	 * @var bool
	 */
									...
									for( var i = 0; i < self._selectOptions[seo].length; i++ ) {
										...
										if( self._selectOptions[seo][i].id ) {
											...
											if( first || typeof cnt[idd] != 'undefined' ) {
												$option.html( self._selectOptions[seo][i].name  +   ( ! first && self._options.showNumberOfProducts ? ' (' + cnt[idd] + ' רע) ' : '' ));
												
												add( $option );
											} else {
												$option.attr('disabled', true).html(  self._selectOptions[seo][i].name + ( self._options.showNumberOfProducts ? '(0 רע)' : '' ) );
	...
	render: function( history ) {
		...
		// usun wszystkie linki do skryptow JS
		self._lastResponse = self._lastResponse.replace( /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '' );
		
		var tmp = jQuery('<tmp>').html( self._lastResponse ),
			content = tmp.find(self._contentId), // znajdz tresc glowna
			json = tmp.find('#mfilter-json'); // informacje JSON zawierajace dane o ilosci produktow wg kategorii
	...
	/**
	 * Sprawdz poprawnosc wpisanego zakresu cen
	 * 
	 * @return bool
	 */
	...
	/**
	 * Przeksztalc parametry z adresu URL na obiekt
	 * 
	 * @return object
	 */
	...
	/**
	 * Pobierz aktualne wartosci filtrow
	 * 
	 * @return object
	 */
	filters: function( alsoEmpty ) {
		...
		// sprawdz czy domyslnie powinna byc zaznaczona opcja "in stock"
	...
	/**
	 * Utworz URL na podstawie parametrow
	 */
	...
	/**
	 * Przeksztalc parametry na adres URL
	 * 
	 * @return string
	 */
